...
def get_budget_insights(user_query, transactions_text):
    ...
    try:
        ...
        return "SynBot is not available because the 'cohere' package is not installed. Install it with: pip install cohere"

    api_key = os.getenv('COHERE_API_KEY')
    if not api_key:
        return "SynBot is not configured: COHERE_API_KEY not found in the environment. Add it to a .env file."

    try:
        co = cohere.Client(api_key)

        system_message = '''You are SynBot, a financial AI assistant developed by Sakshi & Shahu for the Finsight Finance Tracker. Respond to the user in a single, well-structured paragraph, ensuring that all sentences are complete and coherent, without any breaks or cutoff.
